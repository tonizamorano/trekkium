...
add_action('save_post_product', function($post_id){
    ...
    if ( ! empty( $tipo_guardado ) && ( strcasecmp($tipo_guardado, 'Fecha') === 0 || strcasecmp($tipo_guardado, 'fecha') === 0 ) ) {
        ...
        // Usuarios con pedidos que incluyan este producto en estados processing, pending, completed
        ...
        // Incluir la plantilla/función del email y enviar uno por destinatario
        $template_path = get_stylesheet_directory() . '/inc/mailing/emails/email-cambio-fecha.php';
        if ( file_exists( $template_path ) ) {
            include_once $template_path;
        }

        if ( function_exists('trekkium_send_email_cambio_fecha') && ! empty( $recipients ) ) {
